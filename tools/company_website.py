...
# Private / link-local / loopback ranges that must never be fetched.
_BLOCKED_NETWORKS = [
...
def _is_safe_url(url: str) -> bool:
    ...
    try:
        ...
        if not hostname:
            return False
        # Reject bare IP literals that are private before DNS.
        try:
...
def _clean_text(text: str) -> str:
    ...
    joined = "\n".join(nonempty)
    # Keep payloads reasonable for the LLM context.
        ...
        with sync_playwright() as p:
            ...
            try:
                context = browser.new_context(
                    user_agent=(
                        "Mozilla/5.0 (compatible; outbound-copilot/0.1)"
                    )
                )
                page = context.new_page()
                page.goto(url, timeout=timeout * 1000, wait_until="domcontentloaded")
                # Best-effort wait for network idle; do not fail if it times out.
                try:
...
def get_company_website_content(
    ...
    # Stage 1: plain HTTP fetch + HTML extraction
    try:
    ...
    if rendered and len(rendered) > len(text):
        text = rendered  # keep the better of the two for a possible partial return

    # Stage 3: LLM best-effort fallback
    ...
    # Return whatever partial text we managed to collect, even if short.
    return WebsiteFetchResult(
        text=text,
